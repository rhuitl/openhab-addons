...
public class IdentificationMessage {
    ...
    public IdentificationMessage(DataInputStream is) throws IOException {
        ...
        Helper.debug("Identification message 1st Byte Received ", HexConverter.toShortHexString(b));
        ...
        Helper.debug("Manufacture Identification message Byte Received ", manufacturerId);
        
        byte baudRateByte = is.readByte();
        Helper.debug("Baud rate Identification message Byte Received ", HexConverter.toShortHexString(b));
        ...
        Helper.debug("Received after Baud rate", enhancedId);
        
        byte[] identificationBytes = new byte[32];
        int i = 0;
        while (b != '\r') {
            if (i == 32) {
                throw new IOException("Expected carriage return character not received : " + HexConverter.toShortHexString(identificationBytes) 
                + " " + new String(identificationBytes));
            }
