...
public class DataMessage {
    ...
    static DataMessage readModeAbcDataMessage(DataInputStream is, IdentificationMessage identificationMessage)
    	...
    	byte b ;
    	byte[] acknowledgerespondsBytes = new byte[6];
        is.readFully(acknowledgerespondsBytes);
        String responds = new String(acknowledgerespondsBytes, Helper.ASCII_CHARSET);
    	
        if (acknowledgerespondsBytes[0] != 0x06) {
           throw new IOException("Received unexpected data responds for data acknowledgement: " + responds);
        }
        Helper.debug("Acknowledge responds bytes Received ", responds);
...
//      }
        ...
        b = is.readByte();
        responds = HexConverter.toHexString(b) + " "; 
        if (b != '\r') {
        ...
        if (b != bcc.value) {
        	Helper.debug("End of read out tranmission ", responds);
