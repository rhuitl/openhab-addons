...
public class Iec21Port {
    ...
    public DataMessage read() throws IOException, InterruptedIOException {
        ...
        // verify if responds is same as request
        byte[] respondsBytes = new byte[requestMessage.requestMessageBytes.length];
        is.readFully(respondsBytes);
        String responds = new String(respondsBytes, Helper.ASCII_CHARSET);
        if (!Arrays.equals(requestMessage.requestMessageBytes, respondsBytes)) {
            throw new IOException("Received unexpected request responds : " + responds);
         }
        ...
        if (identificationMessage.getProtocolMode() == ProtocolMode.A) {
            int baudRate = identificationMessage.getBaudRate();            
            AcknowledgeMessage acknowledgeMessage = new AcknowledgeMessage(baudRate, ProtocolControlCharacter.NORMAL,
                    AcknowledgeMode.DATA_READOUT);
